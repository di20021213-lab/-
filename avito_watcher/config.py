...
class Settings:
    ...
    # Как часто присылать сводку «я жив», в минутах. 0 — не присылать.
    # Без неё «ничего не приходит» снаружи выглядит одинаково и когда объявлений
    # правда нет, и когда бот упал или Авито его не пускает.
    heartbeat_minutes: int = 360
...
def match_model(title: Optional[str], rules: list[ModelRule]) -> Optional[ModelRule]:
    ...
    Совпадение обязано начинаться с границы слова И не продолжаться цифрой.
    Обе половины появились из-за реальных промахов на живой выдаче:

      · без границы в начале «580» ловилось внутри «B580», и Intel ARC B580
        судилась по потолку RX 580;
      · без запрета цифры «570» ловилось внутри «5700», и RX 5700 XT попадала
        в статистику RX 570 — медиана этой модели выходила 14000 вместо
        реальных нескольких тысяч. То же с «560» и RX 5600 XT.

    Запрет именно на цифру, а не полная граница слова: образцы вроде «1060 6»
    должны ловить «1060 6GB», где дальше идёт буква.
    """
    text = (title or "").lower()
    for rule in rules:
        if any(re.search(r"\b" + re.escape(m) + r"(?!\d)", text) for m in rule.match):
            return rule
    return None
...
def load_settings(config_path: str = "config.yaml") -> Settings:
    ...
    if not chat_id:
        raise ConfigError(
            "Не задан TELEGRAM_CHAT_ID. Запусти `.venv/bin/python get_chat_id.py`, напиши боту, "
