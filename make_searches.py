...
# Чужие платформы. Широкий запрос вроде «коллекционное издание» Авито понимает
# буквально и приносит что угодно: первым же уловом пришла Uncharted 2 для PS3
# из Тулы. Платформы в запросе не задать — у Авито нет отрицания в q=, — так
# что отсекаем у себя.
#
# Отсекаем ЧУЖИЕ, а не требуем «ps4» или «ps5» в заголовке: половина настоящих
# объявлений платформу вообще не пишет («Star wars: Dark Forces Remaster LRG
# #107 Новая»), и требование её потеряло бы. А вот когда PS3 названа прямо —
# это уже не наш диск.
#
# Нужны Switch или Xbox — убери их отсюда и пересобери конфиг.
EXCLUDE_PLATFORMS = [
    "ps3", "ps 3", "playstation 3", "ps2", "ps 2", "playstation 2",
    "ps1", "ps 1", "playstation 1", "ps one", "psone",
    "psp", "ps vita", "psvita",
    "xbox", "x box", "nintendo", "switch", "3ds", "wii",
    "sega", "dreamcast", "gamecube", "dendy",
    # Кириллицей платформу пишут реже, но пишут. Слитные формы («пс3») берём,
    # раздельные («пс 3») — нет: «диски пс 3 шт» это три штуки, а не PS3, и
    # потерять такое объявление хуже, чем получить лишнее.
    "плейстейшн 3", "плейстейшен 3", "плейстейшн 2", "плейстейшен 2",
    "пс3", "пс2", "пс1",
]

# Перекупы и комиссионки. Заголовок у них как у частника — «Star Wars Dark
# Forces Remastered PS5 (ppsa-19729)», не придерёшься, — а вот вокруг заголовка
# в карточке видно скупку: «скупаем приставки и диски», адрес и режим работы.
# Текст карточки Авито отдаёт вместе с выдачей, так что проверка бесплатна.
#
# Список намеренно узкий: только то, что частник о себе не напишет. «Магазин»
# или «обмен» сюда не годятся — их легко встретить в честном объявлении
# («заберёте у магазина»), а потерять нужное хуже, чем получить лишнее.
#
# Посмотреть, что вообще лежит в карточке, и дополнить список по факту:
#     .venv/bin/python -m avito_watcher.main --check --cards
EXCLUDE_SELLERS = [
    "скупаем", "скупка", "скуплю", "выкуп",
    "комиссионный", "комиссионк",
    "наш магазин", "нашем магазине", "сеть магазинов",
    "режим работы", "trade-in", "трейд-ин",
]
...
def block(title: str, top: int, bottom: int, region: str) -> str:
    ...
    #
    # first_run: send — на первом запуске прислать всё подходящее, что УЖЕ
    # лежит, а не молча запомнить. По умолчанию бот запоминает: для широкой
    # категории иначе прилетит сотня старых объявлений разом. Но здесь фильтры
    # узкие, подходящих единицы, и это ровно те объявления, ради которых поиск
    # и заводили. Молча их проглотить — значит начать с пустого места.
    first_run: send
    on_broken: flag
    # Страницу объявления не открываем: это главный расход бюджета адреса.
    check_description: false
    keywords: ["{title.lower()}"]
    exclude_keywords: {yaml_list(EXCLUDE + EXCLUDE_PLATFORMS)}
    # Стоп-слова по тексту карточки, а не по заголовку: отсекают скупки и
    # комиссионки, у которых заголовок неотличим от частного.
    exclude_card_keywords: {yaml_list(EXCLUDE_SELLERS)}
