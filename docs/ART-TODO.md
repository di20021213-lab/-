...
- **Белые ноги — на цветном фоне.** Белая нога без контура на белом фоне
  сливается с ним, и её съедает ещё при снятии фона: так вышло с
  ярославской коровой. Для белоногих просить `plain flat light blue background`.
- Если тень под ногами бежевая, а ноги или носочки белые — впиши породу в
  `assets-src/art/white-legs.txt`: у земли тогда гасится только то, что
  темнее почти-белого.
...
Всего животных 26, картинки есть у 23. Семь новых пород добавлены в
...
Присланные кладём в `assets-src/art/rooms/<id>.jpg` и ставим командой
`python3 tools/import-art.py assets-src/art/rooms/<id>.jpg <id> --room`:
она только ужимает и сохраняет, ничего не вырезая — интерьер лежит
позади живности целиком. Что прислано, то `make-scene.py` не перерисовывает.

Квадратная картинка подходит: панель кадрирует её по низу, и при
пропорции 4:3 окно остаётся в кадре.

Файлы: `public/img/iso/room-<id>.jpg`. Живность стоит на
...
Где на фоне лежит подстилка, записано в `AISLE_GEO` в `game.js`: по ней
живность раскладывается по двое в ряд, если солома достаточно широкая, и
по одному у перил, где сужается. **Сменили фон — переснимите разметку**,
иначе скотина встанет на перила: наложите на картинку сетку в процентах
и впишите, где у дальней и ближней стены кончается солома.
...
Генератор это указание пропускает: в присланное гнездо он всё-таки
положил два бирюзовых яйца. Лишнее убирается флагом импортёра
`--erase-hue <тон>` — пятно заданного тона заливается цветом кольца
вокруг него. Тон смотрим по картинке, у яиц он был 124:

```
python3 tools/import-art.py assets-src/art/props/nest.jpg nest --prop --erase-hue 124
```
...
Присланная постройка сама по себе читается наклейкой, и `--house` доводит
её до набора двумя шагами. Первый — тёмный контур по силуэту тем же цветом,
каким обведены кусты, забор и реквизит: без него у постройки свой контур,
тонкий и тёплый, и она выпадает из сцены. Второй — тень по её собственному
размеру: общая рассчитана на спрайт в 200 пикселей и под постройкой в 420
превращается в невидимое пятнышко.
