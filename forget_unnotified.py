...
def _consequence(label: str, left: int, searches: dict | None) -> list[str]:
    ...
    # Пустой ярлык бот считает первым запуском. Что он тогда сделает — зависит
    # от настройки first_run, и предупреждение обязано это учитывать: раньше
    # оно заявляло «не пришлёт ничего» даже там, где поиск настроен слать.
    if getattr(search, "first_run", "seed") == "send":
        return ["Присланных не остаётся — бот сочтёт это первым запуском, и у "
                "поиска стоит first_run: send, так что пришлёт всё подходящее, "
                "что сейчас лежит на Авито. Это и есть то, зачем обычно запускают "
                "с --all."]
    if search.max_age_minutes is None:
        return ["ВНИМАНИЕ: не остаётся ни одной присланной записи, у поиска нет "
                "max_age, а first_run: seed. Бот сочтёт это первым запуском, молча "
                "запомнит всю выдачу и не пришлёт ничего. Поставь поиску "
                "first_run: send — или оставь хотя бы одну запись."]
