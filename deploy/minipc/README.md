...
```bash
cd ~/avito-watcher
KEENETIC_PASSWORD='пароль от роутера' ./deploy/minipc/wan-bounce.sh
```

Без имени интерфейса он просто покажет список — найди свой WAN (обычно `PPPoE0`
или `ISP`) и запусти уже по-настоящему, **отвязанно от SSH**: интернет пропадёт,
и сессия оборвётся раньше, чем скрипт дойдёт до конца.

```bash
KEENETIC_PASSWORD='пароль' setsid nohup ./deploy/minipc/wan-bounce.sh PPPoE0 300 &
```
