...
> Ставишь систему с нуля? Начни с [INSTALL_UBUNTU.md](INSTALL_UBUNTU.md).
...
Это текстовый доступ: терминал, логи, команды. Увидеть страницу капчи и ткнуть
в неё пальцем так нельзя — для этого поверх того же туннеля поднимается
экран, см. [REMOTE_SCREEN.md](REMOTE_SCREEN.md).
...
## Смена домашнего IP: `wan-bounce.sh`

Авито лимитирует по IP, и иногда адрес нужно сменить. Короткая перезагрузка
роутера часто возвращает тот же самый — у провайдера адрес держится за сессией.
Помогает разрыв на несколько минут, а обесточить роутер удалённо нельзя:
включать будет некому.

Выход — положить не питание, а WAN. Роутер остаётся включённым, локальная сеть
работает, и мини-ПК всё это время видит его по LAN, чтобы вернуть связь.

Скрипт для Keenetic (telnet у него включён по умолчанию):

```bash
cd ~/avito-watcher
KEENETIC_PASSWORD='пароль от роутера' python3 deploy/minipc/wan_bounce.py
```

Без имени интерфейса он покажет таблицу маршрутов — WAN тот, через который идёт
`0.0.0.0/0` (обычно `ISP` или `PPPoE0`). Дальше запускай по-настоящему,
**отвязанно от SSH**: интернет пропадёт, и сессия оборвётся раньше, чем скрипт
дойдёт до перезагрузки.

```bash
KEENETIC_PASSWORD='пароль' setsid nohup python3 deploy/minipc/wan_bounce.py ISP 300 &
```

Через 6–8 минут переподключайся и смотри результат:

```bash
cat ~/wan-bounce.log
```

**Почему не bash и telnet.** Первая версия разделяла шаги паузами, а Keenetic
печатает баннер и приглашение `Login:` с задержкой: логин уходил в пустоту,
пароль попадал в поле логина, вход не удавался — и скрипт молча простаивал
пять минут, ничего не сделав. Здесь клиент дожидается самих приглашений.

**Почему это безопасно.** Финалом идёт `system reboot`, а не «поднять интерфейс».
Keenetic не сохраняет изменения конфигурации без явной команды, поэтому
перезагрузка возвращает WAN во включённое состояние сама — даже если что-то
между командами пойдёт не так. Перезагрузка повторяется до пяти раз, пока
интернет не вернётся.
